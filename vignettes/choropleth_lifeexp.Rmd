...
A common map-based data visualization is a choropleth map. These are maps that use colour or shading to represent the value of a statistic in a region. This vignette will run through an example of using the {bcmaps} boundary information to plot life expectancy in the Health Service Delivery Areas (HSDAs) of British Columbia.
...
Our next step is to create a dataframe with the life expectancy values for each of the HSDAs. 

In the code below, we create a tibble dataframe with the HSDA code numbers and names, and the 2015–2017 life expectancy values published by Statistics Canada.  ^[Statistics Canada. [Table 13-10-0389-01  Life expectancy, at birth and at age 65, by sex, three-year average, Canada, provinces, territories, health regions and peer groups](https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=1310038901)]

In real life this data would probably be in a CSV or Excel file that we would import, or we could use the [{cansim}]( https://CRAN.R-project.org/package=cansim) package to access the data directly from Statistics Canada.
...
To plot the map with the with default colours, we can use `ggplot()`, and `geom_sf()` from the {ggplot2} package:
...
All of the maps in the `bcmaps` package are in BC Albers projection ([EPSG:3005](https://epsg.io/3005)), which is the B.C. government standard. It is an 'equal area' projection, meaning that the size of areas is not distorted, and thus is suitable for analyses on large areas. We can specify that projection for our map using `coord_sf(datum = )` as part of the `ggplot()` function.

In this example, the colour scheme is also changed so that it goes from light to dark, using the blue of the British Columbia flag as the darkest value. This will be used to represent the region with the highest life expectancy.
